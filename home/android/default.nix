  ...
  androidScript = pkgs.writeShellScriptBin "android" ''
    ...
    ${lib.optionalString (apkFiles != []) ''
      ...
      ${lib.concatMapStringsSep "\n" (name: ''
          _pkg=$(${pkgs.aapt}/bin/aapt2 dump packagename ${apkDir}/${name} 2>/dev/null)
          if [ -z "$_pkg" ] || ! echo "$_installed" | grep -qxF "package:$_pkg"; then
            ${adb} install -r -g ${apkDir}/${name} 2>/dev/null \
              && log "  Installed ${name}" \
    ...
    ${lib.optionalString (downloadFiles != []) ''
      ...
      ${lib.concatMapStringsSep "\n" (name: ''
          if ! ${adb} shell "[ -f /sdcard/Download/${name} ]" 2>/dev/null; then
            ${adb} push ${downloadDir}/${name} /sdcard/Download/${name}
            log "  Pushed ${name}"
          fi
        '')
        downloadFiles}
